...
const CallNotification = () => {
    var _a;
    const { ongoingCall, handleJoinCall, handleHangup, pendingOffer } = useSocket();
    if (!ongoingCall ||
    ...
    return (<div className="absolute z-50 bg-slate-500 bg-opacity-70 w-screen h-screen top-0 left-0 flex items-center justify-center">
      <div className="bg-white min-w-[300px] min-h-[100px] flex flex-col items-center justify-center rounded p-4">
        <div className="flex flex-col items-center">
          ...
          <h3>
            {(_a = ongoingCall.participants.caller.profile.fullName) === null || _a === void 0 ? void 0 : _a.split(" ")[0]}
          </h3>
        </div>
        <p className="text-sm mb-2">Incoming Call</p>
        <div className="flex gap-8">
          <button onClick={() => handleJoinCall(ongoingCall)} disabled={!pendingOffer} // disable until the offer is available
     className="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center text-white disabled:opacity-50">
        ...
        {!pendingOffer && (<p className="mt-2 text-sm text-gray-600">Waiting for offer…</p>)}
      </div>
    </div>);
};
