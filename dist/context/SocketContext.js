...
export const SocketContextProvider = ({ children, }) => {
    ...
    const currentSocketUser = onlineUsers === null || onlineUsers === void 0 ? void 0 : onlineUsers.find((u) => u.userId === (user === null || user === void 0 ? void 0 : user.id));
    ...
    const handleJoinCall = useCallback(async (incomingCall) => {
        if (!socket)
            return;
        setIsCallEnded(false);
        // Update state to mark the call as no longer ringing.
        setOngoingCall(Object.assign(Object.assign({}, incomingCall), { isRinging: false }));
        const stream = await getMediaStream();
        if (!stream)
            return;
        const pc = createPeerConnection();
        ...
        if (!pendingOffer) {
            console.error("Callee: No pending offer available! Cannot answer call.");
            return;
        }
        try {
    ...
    const handleHangup = useCallback((data = {}) => {
        if (socket && user && (data === null || data === void 0 ? void 0 : data.ongoingCall) && (data === null || data === void 0 ? void 0 : data.isEmitHangup)) {
            socket.emit("hangup", {
    ...
    useEffect(() => {
        ...
        socket.on("incomingCall", (callData) => {
            ...
            setOngoingCall(Object.assign(Object.assign({}, callData), { isRinging: true }));
        });
        socket.on("hangup", () => {
